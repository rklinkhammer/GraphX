...
nlohmann::json DefaultGraphMetricsJson() {
  return nlohmann::json{{"total_items_processed", 0},
...
nlohmann::json GraphMetricsJson(const graph::GraphMetrics &metrics) {
  return nlohmann::json{{"total_items_processed", metrics.total_items_processed.load()},
...
std::shared_ptr<graph::NodeFacadeAdapterWrapper> TryGetWrapper(
    const std::shared_ptr<graph::INode> &node) {
  return std::dynamic_pointer_cast<graph::NodeFacadeAdapterWrapper>(node);
}

std::string NodeName(const std::shared_ptr<graph::INode> &node, std::size_t index) {
...
std::shared_ptr<graph::IDiagnosable> TryGetDiagnosable(
    const std::shared_ptr<graph::INode> &node) {
  if (const auto wrapper = TryGetWrapper(node); wrapper) {
...
std::string ActivityState(bool thread_active, std::uint64_t rejected_messages,
                          ...
                          std::uint64_t dequeued_messages,
                          bool initialized, bool started) {
  if (thread_active) {
    return "running";
  }
...
nlohmann::json GraphSnapshotCollector::GetMetricsSnapshot() const {
  ...
  const auto graph_manager = runtime_session->GetActiveGraphManager();
  if (!graph_manager) {
    return snapshot;
  }
  ...
  for (std::size_t edge_index = 0; edge_index < edges.size(); ++edge_index) {
    ...
    const auto activity_state =
        ActivityState(thread_active, messages_rejected, backpressure_events,
    ...
    if (metadata->source_node_id < node_totals.size()) {
      ...
      source.peak_queue_depth = std::max(source.peak_queue_depth, peak_queue_depth);
    ...
    snapshot["edges"].push_back(
        {{"edge_index", edge_index},
         ...
         {"source_node_name",
          metadata->source_node_id < nodes.size()
              ? NodeName(nodes[metadata->source_node_id], metadata->source_node_id)
              : std::string{}},
         ...
         {"message_type",
          metadata->message_type_demangled.empty() ? metadata->message_type_name
  ...
  for (std::size_t node_index = 0; node_index < nodes.size(); ++node_index) {
    ...
    const auto activity_state = ActivityState(
        totals.thread_active, totals.rejected_messages,
        totals.backpressure_events, totals.outbound_messages,
        totals.inbound_messages, totals.connected_edges > 0, totals.thread_active);
    snapshot["nodes"].push_back(
        {{"node_index", node_index},
         ...
         {"diagnostics_available", static_cast<bool>(TryGetDiagnosable(nodes[node_index]))},
         {"activity_state", activity_state},
         {"activity_color", ActivityColor(activity_state)}});
  }
