...
#if ACCELGRAPH_ENABLE_METAL && ACCELGRAPH_METAL_HEADERS_AVAILABLE && defined(__APPLE__)
...
std::string NsStringToStdString(const NS::String* value) {
    if (value == nullptr || value->utf8String() == nullptr) {
        return {};
    }
...
class MetalAcceleratorSession final : public IAcceleratorSession {
public:
    explicit MetalAcceleratorSession(AcceleratorSessionInfo info, MTL::Device* device)
        : info_(std::move(info)), device_(device) {}

    ~MetalAcceleratorSession() override {
        std::scoped_lock<std::mutex> lock(mutex_);
        for (auto& [_, buffer] : device_allocations_) {
            if (buffer != nullptr) {
                buffer->release();
            }
        }
        ...
        for (auto& [_, completion] : completions_) {
            if (completion.command_buffer != nullptr) {
                completion.command_buffer->release();
            }
            if (completion.staging_buffer != nullptr) {
                completion.staging_buffer->release();
            }
        }
        device_allocations_.clear();
        queues_.clear();
    ...
    AllocateHost(const HostAllocationRequest& request) override {
        ...
        try {
            ...
            return std::unexpected(MakeError(
                info_, AcceleratorErrorCategory::AllocationFailed, "AllocateHost",
                std::string(kMetalSessionCreationFailureDiagnostic) +
                    " host allocation failed due to insufficient memory"));
        }
    ...
    WriteHost(const HostAllocationHandle& handle, const HostWriteRequest& request) override {
...

        {
            ...
            if (request.source.size() > it->second.size() - request.destination_offset) {
                return std::unexpected(MakeError(
                    info_, AcceleratorErrorCategory::TransferFailed, "WriteHost",
                    std::string(kMetalTransferFailureDiagnostic) +
                        " write range exceeds allocation bounds"));
            }
    ...
    ReadHost(const HostAllocationHandle& handle, const HostReadRequest& request) override {
        ...
        HostReadResult result;
        {
            ...
            if (request.byte_size > it->second.size() - request.source_offset) {
                return std::unexpected(MakeError(
                    info_, AcceleratorErrorCategory::TransferFailed, "ReadHost",
                    std::string(kMetalTransferFailureDiagnostic) +
                        " read range exceeds allocation bounds"));
            }
    ...
    AllocateDevice(const DeviceAllocationRequest& request) override {
        ...
        auto* buffer = device_ != nullptr
                           ? device_->newBuffer(request.byte_size, MTL::ResourceStorageModePrivate)
                           : nullptr;
        if (buffer == nullptr) {
            return std::unexpected(
                MakeError(info_, AcceleratorErrorCategory::AllocationFailed, "AllocateDevice",
                          std::string(kMetalSessionCreationFailureDiagnostic) +
                              " failed to create Metal device buffer"));
        }
        ...
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            device_allocations_.emplace(state->internal_id, buffer);
        }
    ...
    std::expected<QueueAcquisitionResult, AcceleratorError>
    AcquireQueue(const QueueRequest& request) override {
        auto* queue = device_ != nullptr ? device_->newCommandQueue() : nullptr;
        if (queue == nullptr) {
            return std::unexpected(MakeError(
                info_, AcceleratorErrorCategory::QueueFailed, "AcquireQueue",
                std::string(kMetalSessionCreationFailureDiagnostic) +
        ...
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            queues_.emplace(state->internal_id, queue);
        }
    ...
    EnqueueHostToDevice(const HostAllocationHandle& source,
                        ...
                        const QueueHandle& queue,
                        const TransferRequest& request) override {
        auto source_valid = ValidateOwnedHandle(source.state_, "EnqueueHostToDevice");
        if (!source_valid) {
            return std::unexpected(source_valid.error());
        }
        auto destination_valid = ValidateOwnedHandle(destination.state_, "EnqueueHostToDevice");
        if (!destination_valid) {
            return std::unexpected(destination_valid.error());
        }
        auto queue_valid = ValidateOwnedHandle(queue.state_, "EnqueueHostToDevice");
        if (!queue_valid) {
            return std::unexpected(queue_valid.error());
        }

        return EnqueueTransfer(source.state_->internal_id,
                               destination.state_->internal_id,
                               queue.state_->internal_id,
                               request,
                               TransferDirection::HostToDevice,
                               source.state_->internal_id,
    ...
    EnqueueDeviceToHost(const DeviceAllocationHandle& source,
                        ...
                        const QueueHandle& queue,
                        const TransferRequest& request) override {
        auto source_valid = ValidateOwnedHandle(source.state_, "EnqueueDeviceToHost");
        if (!source_valid) {
            return std::unexpected(source_valid.error());
        }
        auto destination_valid = ValidateOwnedHandle(destination.state_, "EnqueueDeviceToHost");
        if (!destination_valid) {
            return std::unexpected(destination_valid.error());
        }
        auto queue_valid = ValidateOwnedHandle(queue.state_, "EnqueueDeviceToHost");
        if (!queue_valid) {
            return std::unexpected(queue_valid.error());
        }
    ...
    Wait(const TransferCompletion& completion, const WaitRequest& request) override {
        ...
        PendingCompletion pending_snapshot;
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            auto it = completions_.find(completion.state_->internal_id);
            ...
            pending_snapshot = it->second;
        }

        if (pending_snapshot.command_buffer == nullptr) {
            return std::unexpected(
                MakeError(info_, AcceleratorErrorCategory::TransferFailed, "WaitCompletion",
                          std::string(kMetalTransferFailureDiagnostic) +
                              " missing command buffer"));
        }

        pending_snapshot.command_buffer->waitUntilCompleted();

        if (pending_snapshot.command_buffer->status() == MTL::CommandBufferStatusError) {
            return std::unexpected(
                MakeError(info_, AcceleratorErrorCategory::TransferFailed, "WaitCompletion",
                          std::string(kMetalTransferFailureDiagnostic) +
        ...
        if (pending_snapshot.direction == TransferDirection::DeviceToHost &&
            !pending_snapshot.copied_to_host) {
            std::scoped_lock<std::mutex> lock(mutex_);
            auto host_it = host_allocations_.find(pending_snapshot.host_allocation_internal_id);
            if (host_it == host_allocations_.end()) {
                return std::unexpected(
                    MakeError(info_, AcceleratorErrorCategory::TransferFailed, "WaitCompletion",
                              std::string(kMetalTransferFailureDiagnostic) +
                                  " host allocation became inactive before completion"));
            }
            if (pending_snapshot.staging_buffer == nullptr || pending_snapshot.byte_size == 0) {
                return std::unexpected(
                    MakeError(info_, AcceleratorErrorCategory::TransferFailed, "WaitCompletion",
                              std::string(kMetalTransferFailureDiagnostic) +
                                  " staging buffer missing for D2H completion"));
            }
            std::memcpy(host_it->second.data() + pending_snapshot.host_destination_offset,
                        pending_snapshot.staging_buffer->contents(),
                        pending_snapshot.byte_size);

            auto completion_it = completions_.find(completion.state_->internal_id);
            if (completion_it != completions_.end()) {
                completion_it->second.copied_to_host = true;
            }
        }
    ...
    Release(const TransferCompletion& completion, const ReleaseRequest& request) override {
        ...
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            const auto completion_it = completions_.find(completion.state_->internal_id);
            if (completion_it != completions_.end()) {
                pending = completion_it->second;
                completions_.erase(completion_it);
                found_completion = true;
            }

            const auto event_it = events_.find(completion.state_->event_internal_id);
        ...
        if (!found_completion && !request.allow_if_released) {
            return std::unexpected(MakeError(info_,
                                             AcceleratorErrorCategory::EventFailed,
                                             "ReleaseCompletion",
        ...
        if (pending.command_buffer != nullptr) {
            pending.command_buffer->release();
        }
        if (pending.staging_buffer != nullptr) {
            pending.staging_buffer->release();
        }
        ...
        ReleaseResult result;
        result.released = found_completion;
        return result;
    }
...
private:
    ...
    struct PendingCompletion {
        ...
        std::uint64_t host_allocation_internal_id{0};
        std::size_t host_destination_offset{0};
    ...
    std::expected<ReleaseResult, AcceleratorError>
    ReleaseSetCommon(const std::shared_ptr<DeviceAllocationHandle::State>& state,
                     const ReleaseRequest& request,
                     const std::string& operation,
                     std::unordered_map<std::uint64_t, MTL::Buffer*>& storage) {
        auto valid = ValidateOwnedHandle(state, operation, request.allow_if_released);
        if (!valid) {
            return std::unexpected(valid.error());
        }

        MTL::Buffer* removed_buffer = nullptr;
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            const auto it = storage.find(state->internal_id);
            if (it != storage.end()) {
                removed_buffer = it->second;
                storage.erase(it);
            }
        }

        if (removed_buffer == nullptr && !request.allow_if_released) {
            return std::unexpected(MakeError(info_,
                                             AcceleratorErrorCategory::InvalidState,
                                             operation,
                                             "resource was not active"));
        ...
        if (removed_buffer != nullptr) {
            removed_buffer->release();
        }
        state->released.store(true, std::memory_order_release);

        ReleaseResult result;
        result.released = removed_buffer != nullptr;
        return result;
    }

    std::expected<ReleaseResult, AcceleratorError>
    ReleaseSetCommon(const std::shared_ptr<QueueHandle::State>& state,
                     const ReleaseRequest& request,
                     const std::string& operation,
                     std::unordered_map<std::uint64_t, MTL::CommandQueue*>& storage,
                     AcceleratorErrorCategory missing_category) {
        auto valid = ValidateOwnedHandle(state, operation, request.allow_if_released);
        if (!valid) {
            return std::unexpected(valid.error());
        ...
        MTL::CommandQueue* removed_queue = nullptr;
        {
            std::scoped_lock<std::mutex> lock(mutex_);
            const auto it = storage.find(state->internal_id);
            if (it != storage.end()) {
                removed_queue = it->second;
                storage.erase(it);
            }
        }

        if (removed_queue == nullptr && !request.allow_if_released) {
            return std::unexpected(MakeError(info_,
                                             missing_category,
                                             operation,
                                             "resource was not active"));
        ...
        if (removed_queue != nullptr) {
            removed_queue->release();
        }
        state->released.store(true, std::memory_order_release);

        ReleaseResult result;
        result.released = removed_queue != nullptr;
        return result;
    }

    std::expected<TransferResult, AcceleratorError>
    EnqueueTransfer(std::uint64_t source_internal_id,
                    std::uint64_t destination_internal_id,
                    std::uint64_t queue_internal_id,
                    const TransferRequest& request,
                    TransferDirection direction,
                    std::uint64_t host_allocation_internal_id,
                    std::size_t host_destination_offset,
                    std::size_t host_source_offset,
                    const std::string& operation) {
        if (request.byte_size == 0) {
            return std::unexpected(
                MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                          std::string(kMetalTransferFailureDiagnostic) +
                              " transfer byte_size must be greater than zero"));
        }

        MTL::CommandQueue* queue = nullptr;
        MTL::Buffer* source_device_buffer = nullptr;
        MTL::Buffer* destination_device_buffer = nullptr;
        MTL::Buffer* staging_buffer = nullptr;
        std::vector<std::byte> host_snapshot;

        {
            std::scoped_lock<std::mutex> lock(mutex_);

            const auto queue_it = queues_.find(queue_internal_id);
            if (queue_it == queues_.end() || queue_it->second == nullptr) {
                return std::unexpected(
                    MakeError(info_, AcceleratorErrorCategory::QueueFailed, operation,
                              std::string(kMetalTransferFailureDiagnostic) +
                                  " queue is not active"));
            }
            ...
            if (direction == TransferDirection::HostToDevice) {
                const auto host_it = host_allocations_.find(source_internal_id);
                if (host_it == host_allocations_.end()) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " source host allocation is not active"));
                }
                if (host_source_offset > host_it->second.size() ||
                    request.byte_size > host_it->second.size() - host_source_offset) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " source range exceeds allocation bounds"));
                }
                host_snapshot.assign(host_it->second.begin() + static_cast<std::ptrdiff_t>(host_source_offset),
                                     host_it->second.begin() +
                                         static_cast<std::ptrdiff_t>(host_source_offset + request.byte_size));

                const auto device_it = device_allocations_.find(destination_internal_id);
                if (device_it == device_allocations_.end() || device_it->second == nullptr) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " destination device allocation is not active"));
                }
                destination_device_buffer = device_it->second;
                if (request.destination_offset > destination_device_buffer->length() ||
                    request.byte_size > destination_device_buffer->length() - request.destination_offset) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " destination range exceeds allocation bounds"));
                }
            } else {
                const auto device_it = device_allocations_.find(source_internal_id);
                if (device_it == device_allocations_.end() || device_it->second == nullptr) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " source device allocation is not active"));
                }
                source_device_buffer = device_it->second;
                if (host_source_offset > source_device_buffer->length() ||
                    request.byte_size > source_device_buffer->length() - host_source_offset) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " source range exceeds allocation bounds"));
                ...
                const auto host_it = host_allocations_.find(destination_internal_id);
                if (host_it == host_allocations_.end()) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " destination host allocation is not active"));
                }
                if (host_destination_offset > host_it->second.size() ||
                    request.byte_size > host_it->second.size() - host_destination_offset) {
                    return std::unexpected(
                        MakeError(info_, AcceleratorErrorCategory::TransferFailed, operation,
                                  std::string(kMetalTransferFailureDiagnostic) +
                                      " destination range exceeds allocation bounds"));
        ...
        staging_buffer =
            device_ != nullptr ? device_->newBuffer(request.byte_size, MTL::ResourceStorageModeShared) : nullptr;
        ...
        if (direction == TransferDirection::HostToDevice) {
            std::memcpy(staging_buffer->contents(), host_snapshot.data(), request.byte_size);
        }
        ...
        if (direction == TransferDirection::HostToDevice) {
            blit->copyFromBuffer(staging_buffer,
                                 0,
                                 destination_device_buffer,
                                 request.destination_offset,
                                 request.byte_size);
        } else {
            blit->copyFromBuffer(source_device_buffer,
                                 host_source_offset,
                                 staging_buffer,
                                 0,
                                 request.byte_size);
        ...
        MTL::SharedEvent* event = device_ != nullptr ? device_->newSharedEvent() : nullptr;
        if (event == nullptr) {
            command_buffer->release();
            staging_buffer->release();
            return std::unexpected(
        ...
        const std::uint64_t signal_value = NextId();
        command_buffer->encodeSignalEvent(event, signal_value);
...

        {
            ...
            completions_.emplace(completion_state->internal_id,
                                 PendingCompletion{
                                     ...
                                     .event_internal_id = event_state->internal_id,
                                     .signal_value = signal_value,
                                     .direction = direction,
                                     .host_allocation_internal_id = host_allocation_internal_id,
                                     .host_destination_offset = host_destination_offset,
    ...
    mutable std::mutex mutex_;
    std::unordered_map<std::uint64_t, std::vector<std::byte>> host_allocations_;
    std::unordered_map<std::uint64_t, MTL::Buffer*> device_allocations_;
    std::unordered_map<std::uint64_t, MTL::CommandQueue*> queues_;
    std::unordered_map<std::uint64_t, MTL::SharedEvent*> events_;
    std::unordered_map<std::uint64_t, PendingCompletion> completions_;
};

MetalAcceleratorProvider::MetalAcceleratorProvider() {
    info_.provider_id = AcceleratorProviderId{"metal.default"};
    info_.backend = AcceleratorBackend::Metal;
    ...
    for (NS::UInteger index = 0; index < devices->count(); ++index) {
        auto* device = static_cast<MTL::Device*>(devices->object(index));
        if (device == nullptr) {
            continue;
        }

        AcceleratorDeviceInfo device_info;
        device_info.id = AcceleratorDeviceId{"metal:" + std::to_string(index)};
        device_info.name = NsStringToStdString(device->name());
        if (device_info.name.empty()) {
            device_info.name = "Apple Metal Device " + std::to_string(index);
        }
        device_info.backend = AcceleratorBackend::Metal;
        info_.devices.push_back(std::move(device_info));
    }

    devices->release();
}

AcceleratorProviderInfo MetalAcceleratorProvider::Info() const {
    return info_;
}

std::expected<std::shared_ptr<IAcceleratorSession>, AcceleratorError>
MetalAcceleratorProvider::CreateSession(const AcceleratorSessionCreateRequest& request) {
    if (info_.devices.empty()) {
        NS::Array* devices = MTL::CopyAllDevices();
        if (devices != nullptr) {
            devices->release();
            return std::unexpected(MakeProviderError(info_,
                                                     ...
                                                     "CreateSession",
                                                     kMetalNoCompatibleDeviceDiagnostic,
        ...
        return std::unexpected(MakeProviderError(info_,
                                                 AcceleratorErrorCategory::Unavailable,
                                                 "CreateSession",
                                                 kMetalRuntimeUnavailableDiagnostic,
                                                 request.requested_device));
    }
    ...
    if (request.requested_device.has_value()) {
        bool matched = false;
        for (std::size_t index = 0; index < info_.devices.size(); ++index) {
            if (info_.devices[index].id == request.requested_device.value()) {
    ...
    NS::Array* devices = MTL::CopyAllDevices();
    auto* device = devices != nullptr && selected_index < devices->count()
                       ? static_cast<MTL::Device*>(devices->object(selected_index))
                       : nullptr;
    if (device != nullptr) {
        device->retain();
    }
    ...
    if (device == nullptr) {
        return std::unexpected(MakeProviderError(
            info_, AcceleratorErrorCategory::BackendFailure, "CreateSession",
            std::string(kMetalSessionCreationFailureDiagnostic) +
                " Metal device acquisition failed",
            request.requested_device));
    }
    ...
    session_info.backend = info_.backend;
    session_info.execution_mode = info_.execution_mode;
    session_info.device = info_.devices[selected_index];

    return std::make_shared<MetalAcceleratorSession>(std::move(session_info), device);
}

#elif ACCELGRAPH_ENABLE_METAL
...
MetalAcceleratorProvider::CreateSession(const AcceleratorSessionCreateRequest& request) {
    ...
    if (!on_apple_platform || !ACCELGRAPH_METAL_HEADERS_AVAILABLE) {
        return std::unexpected(MakeProviderError(info_,
                                                 AcceleratorErrorCategory::Unavailable,
                                                 "CreateSession",
                                                 kMetalRuntimeUnavailableDiagnostic,
    ...
    return std::unexpected(MakeProviderError(
        info_, AcceleratorErrorCategory::BackendFailure, "CreateSession",
        std::string(kMetalSessionCreationFailureDiagnostic) +
            " no Metal implementation compiled for this host/toolchain",
        request.requested_device));
}
...
MetalAcceleratorProvider::CreateSession(const AcceleratorSessionCreateRequest& request) {
    return std::unexpected(MakeProviderError(info_,
                                             ...
                                             "CreateSession",
                                             kMetalSupportNotCompiledDiagnostic,
                                             request.requested_device));
}
