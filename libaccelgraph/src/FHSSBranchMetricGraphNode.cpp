...
std::string BuildBackendFallbackDiagnostic(const FHSSAccelConfig& config,
    ...
    if (config.backend == AcceleratorBackend::Metal) {
        return kFhssBranchMetricMetalNativeNotImplementedDiagnostic;
    }
    if (config.backend == AcceleratorBackend::Cuda) {
        return kFhssBranchMetricCudaNativeNotImplementedDiagnostic;
    }
    return {};
}
...
std::optional<std::string> RunCudaSessionProbe(const std::shared_ptr<accelgraph::IAcceleratorSession>& session,
    ...
    std::vector<std::byte> probe_input(kProbeBytes, std::byte{0x51});
...
void AccelFhssBranchMetricNode::Configure(const graph::JsonView& cfg) {
    const auto& json = cfg.Raw();
    if (!json.is_object()) {
        throw graph::ConfigError("AccelFhssBranchMetricNode configuration must be a JSON object");
    }

    accel_config_ = ParseFHSSAccelConfig(cfg);

    cpsm_config_ = dsp::fhss::CPSMDecoderConfig{};
    if (json.contains("symbol_count")) {
        if (!json.at("symbol_count").is_number_unsigned()) {
            throw graph::ConfigError("AccelFhssBranchMetricNode field 'symbol_count' must be an unsigned integer");
        }
        cpsm_config_.symbol_count = json.at("symbol_count").get<std::uint32_t>();
    }
    if (json.contains("modulation_index")) {
        if (!json.at("modulation_index").is_number()) {
            throw graph::ConfigError("AccelFhssBranchMetricNode field 'modulation_index' must be numeric");
        }
        cpsm_config_.modulation_index = json.at("modulation_index").get<double>();
    }
    if (json.contains("initial_phase_state")) {
        if (!json.at("initial_phase_state").is_number_unsigned()) {
            throw graph::ConfigError("AccelFhssBranchMetricNode field 'initial_phase_state' must be an unsigned integer");
        }
        cpsm_config_.initial_phase_state = json.at("initial_phase_state").get<std::uint32_t>();
    }
    if (json.contains("check_terminal_phase")) {
        if (!json.at("check_terminal_phase").is_boolean()) {
            throw graph::ConfigError("AccelFhssBranchMetricNode field 'check_terminal_phase' must be a boolean");
        }
        cpsm_config_.check_terminal_phase = json.at("check_terminal_phase").get<bool>();
    }
    if (json.contains("expected_terminal_phase_state")) {
        if (!json.at("expected_terminal_phase_state").is_number_unsigned()) {
            throw graph::ConfigError("AccelFhssBranchMetricNode field 'expected_terminal_phase_state' must be an unsigned integer");
        }
        cpsm_config_.expected_terminal_phase_state = json.at("expected_terminal_phase_state").get<std::uint32_t>();
    }

    if (auto validation = dsp::fhss::ValidateCPSMDecoderConfig(cpsm_config_); !validation) {
    ...
    if (requested_backend_ == AcceleratorBackend::Metal) {
        ...
        if (session.has_value()) {
            auto probe_error = RunCudaSessionProbe(session.value(), "AccelFhssBranchMetricNode");
...
std::optional<FHSSCpsmBranchMetricToken> AccelFhssBranchMetricNode::Transfer(
    ...
    if (input.sidecar.detected_pulses.empty() || input.sidecar.pulse_evidence.empty()) {
        return std::nullopt;
    }
    if (input.sidecar.detected_pulses.size() != input.sidecar.pulse_evidence.size()) {
        return std::nullopt;
    }

    FHSSCpsmBranchMetricToken output{};
    output.token_id = input.token_id;
    output.sidecar.correlation = input.sidecar.correlation;
    output.sidecar.pulse_metrics.reserve(input.sidecar.detected_pulses.size());

    for (std::size_t i = 0; i < input.sidecar.detected_pulses.size(); ++i) {
        const auto& pulse = input.sidecar.detected_pulses[i];
        const auto& evidence = input.sidecar.pulse_evidence[i];

        if (!dsp::fhss::FHSSGraphXEvidenceHasHostComplexIq(evidence)) {
            continue;
        }

        const auto evidence_samples = dsp::fhss::FHSSGraphXComplexEvidenceSamples(evidence);
        if (evidence_samples.empty()) {
            continue;
        }

        auto metrics = dsp::fhss::CPSMBranchMetricKernel::Compute(evidence_samples, cpsm_config_);
        if (!metrics.has_value()) {
            continue;
        }

        dsp::fhss::FHSSCpsmPulseBranchMetric pulse_metric{};
        pulse_metric.candidate.pulse = pulse;
        pulse_metric.candidate.complex_evidence = evidence;
        pulse_metric.trellis_state_count = dsp::fhss::CPSMPhaseStateCount(cpsm_config_.modulation_index);
...
bool AccelFhssBranchMetricNode::StageInputThroughCuda(const FHSSPerChannelPulseEvidenceToken& input) {
    if (selected_backend_ != AcceleratorBackend::Cuda || !cuda_session_) {
        return true;
    }

    for (const auto& evidence : input.sidecar.pulse_evidence) {
        if (auto staging_error = StageEvidenceThroughCuda(
                cuda_session_, evidence, "AccelFhssBranchMetricNode");
...
AcceleratorBackend AccelFhssBranchMetricNode::RequestedBackend() const noexcept {
    return requested_backend_;
}

AcceleratorBackend AccelFhssBranchMetricNode::SelectedBackend() const noexcept {
    return selected_backend_;
}

bool AccelFhssBranchMetricNode::UsedFallback() const noexcept {
    return used_fallback_;
}

const std::string& AccelFhssBranchMetricNode::FallbackDiagnostic() const noexcept {
    return fallback_diagnostic_;
}

void AccelFhssBranchMetricSinkNode::Configure(const graph::JsonView& cfg) {
    if (!cfg.Raw().is_object()) {
        throw graph::ConfigError("AccelFhssBranchMetricSinkNode configuration must be a JSON object");
    }
    std::scoped_lock<std::mutex> lock(mutex_);
    last_packet_.reset();
}

bool AccelFhssBranchMetricSinkNode::Consume(const FHSSCpsmBranchMetricToken& packet,
                                            std::integral_constant<std::size_t, 0>) {
    std::scoped_lock<std::mutex> lock(mutex_);
    last_packet_ = packet;
    return true;
...
std::optional<FHSSCpsmBranchMetricToken> AccelFhssBranchMetricSinkNode::LastPacket() const {
    std::scoped_lock<std::mutex> lock(mutex_);
    return last_packet_;
}
