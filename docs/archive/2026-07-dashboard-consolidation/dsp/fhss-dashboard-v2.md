...
> **Historical graphical recommendation:** The React Flow, ELK.js, exact-port,
> hierarchy, accessibility, and bounded-activity ideas in this document remain
> useful design input. Its FHSS-specific frontend, API, runtime, configuration,
> and packaging structure is not an implementation authority. The replacement
> plan is
> [`plan/GRAPHX_GENERIC_GRAPHICAL_DASHBOARD_PLAN.md`](../../plan/GRAPHX_GENERIC_GRAPHICAL_DASHBOARD_PLAN.md),
> which extends the existing generic `GraphCoordinator`, `GraphHttpServer`,
> `graphx-dashboard`, `/api/v1` routes, and checked-in `index.html` without a
> parallel management stack.
