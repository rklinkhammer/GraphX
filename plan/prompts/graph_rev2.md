...
- Phase 4 status: topology hardening guardrails and topology-author documentation added on macOS; synthetic topology tests remain in the standard smoke/unit path and backend-unavailable diagnostics remain actionable.
- Phase 5 status: complete on macOS and Jetson; topology anti-bypass guard now auto-discovers `test_accelgraph_phase*.cpp` files and scans topology-style phase tests for forbidden manual configuration tokens while allowlisting explicit non-topology phase unit tests.
- Phase 6 status: complete on macOS and Jetson; libaccelgraph now has explicit CTest discovery coverage mirroring libgraph/libdsp patterns, required topology suites are asserted in `test_libaccelgraph_smoke`, and CUDA-only phase 6b discovery remains gated by `ACCELGRAPH_ENABLE_CUDA`.
- Phase 7 status: complete on macOS and Jetson; synthetic transfer and spectrum topology coverage now explicitly spans CPU/Metal/CUDA linear paths plus strict/allow fallback variants in the contract matrix, with no manual test-side configuration bypasses.
- Phase 8 status: complete on macOS and Jetson; transfer topologies were made truthful by removing disconnected lifecycle nodes where unexercised, connectivity guardrails now enforce no disconnected nodes and sink/source role correctness, and CUDA transfer/spectrum topology checks remain diagnostically accurate.
...
- Phase 4 scope note: legacy checked-in topology formats may coexist with synthetic-node_config topologies, but synthetic coverage remains strictly topology-driven and guardrailed.
- Remaining work before later phases: continue parity/catalog expansion and phase-scoped hardening while keeping loader-owned initialization and backend-accurate diagnostics.
