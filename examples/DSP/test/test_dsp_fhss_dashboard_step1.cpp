...
HttpResponse HttpGet(std::uint16_t port, const std::string &target) {
  ...
  if (body_pos != std::string::npos) {
    std::istringstream header_stream(response.substr(first_line_end + 2,
                                                      body_pos - first_line_end - 2));
    std::string line;
...
HttpResponse HttpRaw(std::uint16_t port, const std::string &wire) {
  ...
  HttpResponse parsed;
  const auto first_line_end = response.find("\r\n");
  if (first_line_end == std::string::npos) return parsed;
  std::istringstream status_line(response.substr(0, first_line_end));
  std::string http;
  status_line >> http >> parsed.status_code;
  const auto body_pos = response.find("\r\n\r\n");
  if (body_pos != std::string::npos) {
    std::istringstream headers(response.substr(first_line_end + 2,
                                               body_pos - first_line_end - 2));
    std::string line;
...
TimedResponse HttpSlowProgress(std::uint16_t port, std::chrono::milliseconds interval,
                               int progress_writes) {
  const int fd = ::socket(AF_INET, SOCK_STREAM, 0);
  if (fd < 0) return {};
#if defined(SO_NOSIGPIPE)
  ...
  for (int index = 0; index < progress_writes; ++index) {
    ...
    if (::send(fd, &progress, 1, 0) != 1) break;
  }
  std::string wire_response;
  std::array<char, 2048> receive_buffer{};
  while (const auto count = ::recv(fd, receive_buffer.data(), receive_buffer.size(), 0)) {
    if (count < 0) break;
    wire_response.append(receive_buffer.data(), static_cast<std::size_t>(count));
  }
  ::close(fd);
  TimedResponse result;
  result.elapsed = std::chrono::steady_clock::now() - started;
  const auto first_line_end = wire_response.find("\r\n");
  if (first_line_end == std::string::npos) return result;
  ...
  if (body_pos != std::string::npos) result.response.body = wire_response.substr(body_pos + 4);
  return result;
}
...
TEST_F(FhssDashboardServerContractTest, ContentTypesAllowAndDefaultMutationAreExact) {
  EXPECT_EQ(HttpGet(server_->BoundPort(), "/").headers.at("content-type"),
            "text/html; charset=utf-8");
  EXPECT_EQ(HttpGet(server_->BoundPort(), "/healthz").headers.at("content-type"),
            "application/json");
  const auto method = HttpRaw(server_->BoundPort(),
  ...
  const auto mutation = HttpRaw(server_->BoundPort(),
...
TEST_F(FhssDashboardServerContractTest, AddsDefensiveBrowserHeaders) {
  ...
  EXPECT_NE(response.headers.at("content-security-policy").find("frame-ancestors 'none'"),
            std::string::npos);
}
...
TEST_F(FhssDashboardServerContractTest, SafeMetricsGetDoesNotPublishAnEvent) {
  ASSERT_EQ(HttpGet(server_->BoundPort(), "/api/v1/fhss/events?client_id=safe-get").status_code, 200);
  ASSERT_EQ(HttpGet(server_->BoundPort(), "/api/v1/fhss/metrics").status_code, 200);
  const auto events = HttpGet(server_->BoundPort(), "/api/v1/fhss/events?client_id=safe-get");
  ASSERT_EQ(events.status_code, 200);
  EXPECT_TRUE(nlohmann::json::parse(events.body).at("events").empty());
}

TEST_F(FhssDashboardServerContractTest, RejectsAmbiguousFramingAndUnsupportedMethods) {
  const auto ambiguous = HttpRaw(
      server_->BoundPort(),
      "POST /api/v1/fhss/config HTTP/1.1\r\nHost: localhost\r\n"
      "Content-Length: 2\r\nTransfer-Encoding: chunked\r\nConnection: close\r\n\r\n{}");
  EXPECT_EQ(ambiguous.status_code, 400);
  const auto unsupported = HttpRaw(
      server_->BoundPort(), "PUT /healthz HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n");
...
TEST_F(FhssDashboardServerContractTest, PartialClientDoesNotBlockConcurrentHealthRequest) {
  const int fd = ::socket(AF_INET, SOCK_STREAM, 0);
  ASSERT_GE(fd, 0);
  sockaddr_in addr{};
  addr.sin_family = AF_INET;
...
TEST_F(FhssDashboardServerContractTest, RejectsSymlinkAndNonRegularAssets) {
  ...
  { std::ofstream stream(outside); stream << "secret"; }
  std::error_code error;
  std::filesystem::create_symlink(outside, assets_ / "escape.txt", error);
  if (!error) EXPECT_EQ(HttpGet(server_->BoundPort(), "/escape.txt").status_code, 404);
...
TEST(FhssDashboardServerContractStandaloneTest,
  ...
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_slow_handler_assets");
  auto config_service = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  ...
  options.application_api_handler =
      ...
      .handler = [cancellation_observed](const graph::dashboard::EmbeddedDashboardServer::ApiRequest &,
                              const graph::dashboard::EmbeddedDashboardServer::ApiContext &context)
      -> std::optional<graph::dashboard::EmbeddedDashboardServer::ApiResponse> {
    ...
    return graph::dashboard::EmbeddedDashboardServer::ApiResponse{
        .status_code = 408, .content_type = "application/problem+json", .body = "{}"};
  }, .cooperative_cancellation = true,
     .maximum_checkpoint_latency = std::chrono::milliseconds(5)};
  graph::dashboard::EmbeddedDashboardServer server(options, config_service, runtime_session,
                                                    snapshot_collector);
...
TEST(FhssDashboardServerFailureTest, RejectsHandlerWithoutCooperativeBoundContract) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_unbounded_handler_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  ...
  graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
  EXPECT_FALSE(server.Start());
  EXPECT_NE(server.LastError().find("cooperative cancellation"), std::string::npos);
  std::error_code error;
  std::filesystem::remove_all(assets, error);
}

TEST(FhssDashboardServerContractStandaloneTest, PortCanBeReusedAfterStop) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_port_reuse_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options first_options;
  first_options.asset_directory = assets;
  auto first_runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  graph::dashboard::EmbeddedDashboardServer first(first_options, config, first_runtime, snapshots);
  ...
  auto second_runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  graph::dashboard::EmbeddedDashboardServer second(second_options, config, second_runtime, snapshots);
...
TEST(FhssDashboardServerContractStandaloneTest, IdleRequestTimesOutWith408) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_idle_timeout_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options options;
  ...
  graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
  ASSERT_TRUE(server.Start()) << server.LastError();
  const auto response = HttpRaw(server.BoundPort(), "GET /healthz HTTP/1.1\r\nHost: localhost\r\n");
  EXPECT_EQ(response.status_code, 408);
...
TEST(FhssDashboardServerContractStandaloneTest,
  ...
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_read_deadline_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options options;
  ...
  graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
  ASSERT_TRUE(server.Start()) << server.LastError();
  const auto result = HttpSlowProgress(server.BoundPort(), std::chrono::milliseconds(40), 5);
  EXPECT_EQ(result.response.status_code, 408);
  EXPECT_EQ(nlohmann::json::parse(result.response.body).at("code"), "read_timeout");
...
TEST(FhssDashboardServerContractStandaloneTest,
  ...
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_total_deadline_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options options;
  ...
  graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
  ASSERT_TRUE(server.Start()) << server.LastError();
  const auto result = HttpSlowProgress(server.BoundPort(), std::chrono::milliseconds(40), 4);
  EXPECT_EQ(result.response.status_code, 408);
  EXPECT_EQ(nlohmann::json::parse(result.response.body).at("code"), "total_request_timeout");
...
TEST(FhssDashboardServerContractStandaloneTest, ResponseLimitAndSiblingEscapeAreRejected) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_response_limit_assets");
  { std::ofstream large(assets / "large.txt"); large << std::string(2048, 'x'); }
  const auto sibling = assets.parent_path() / "graphx_dashboard_response_limit_assets_evil";
  std::filesystem::create_directories(sibling);
  { std::ofstream secret(sibling / "secret.txt"); secret << "secret"; }
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options options;
  ...
  EXPECT_EQ(HttpGet(server.BoundPort(), "/%2e%2e/graphx_dashboard_response_limit_assets_evil/secret.txt").status_code, 404);
  server.Stop();
  std::error_code error;
  std::filesystem::remove_all(assets, error);
...
TEST(FhssDashboardServerContractStandaloneTest, ValidWrongTypeJsonReturns4xxAndServerSurvives) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_wrong_type_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  graph::dashboard::EmbeddedDashboardServer::Options options;
  ...
  graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
  ASSERT_TRUE(server.Start()) << server.LastError();
  const auto disallowed = HttpRaw(server.BoundPort(),
      "POST /api/v1/fhss/config HTTP/1.1\r\nHost: localhost\r\nContent-Length: 2\r\n"
      "Content-Type: application/json\r\nConnection: close\r\n\r\n{}");
...
TEST(FhssDashboardServerFailureTest, RejectsNonLoopbackAndInvalidHosts) {
  const auto assets = MakeTempAssetDirectory("graphx_dashboard_host_failure_assets");
  auto config = std::make_shared<graph::dashboard::GraphConfigurationService>(MinimalGraphConfig());
  auto runtime = std::make_shared<graph::dashboard::GraphRuntimeSession>();
  auto snapshots = std::make_shared<graph::dashboard::GraphSnapshotCollector>();
  for (const std::string host : {"0.0.0.0", "192.0.2.1", "localhost", "not-an-address"}) {
    graph::dashboard::EmbeddedDashboardServer::Options options;
    options.host = host;
    options.asset_directory = assets;
    graph::dashboard::EmbeddedDashboardServer server(options, config, runtime, snapshots);
...
TEST(FhssDashboardServerContractStandaloneTest, SupportsExplicitIpv6LoopbackWhenAvailable) {
...
TEST(FhssDashboardServerContractStandaloneTest, EnforcesHttpAndJsonStructuralLimits) {
