...
class DashboardServerStep2Test : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(DashboardServerStep2Test, PatchRejectsGeneratedFieldsAndStaleRevisions) {
  const auto valid_patch = nlohmann::json{{"schema", "graphx.dashboard.config_update.v1"},
  ...
  const auto first = HttpRequest(server_->BoundPort(), "PATCH", "/api/v1/config", valid_patch.dump());
  EXPECT_EQ(first.status_code, 200) << first.body;
  const auto first_json = nlohmann::json::parse(first.body);
  EXPECT_EQ(first_json.at("schema").get<std::string>(), "graphx.dashboard.config_result.v1");
  ...
  const auto stale_patch = nlohmann::json{{"schema", "graphx.dashboard.config_update.v1"},
                                          {"command_id", "cmd-step2-patch-2"},
                                          {"expected_revision", 1},
                                          {"pointer", "/fhss/scenario/iq_center_frequency_hz"},
                                          {"value", 1240000002.0},
                                          {"apply", "staged"}};
  const auto stale = HttpRequest(server_->BoundPort(), "PATCH", "/api/v1/config", stale_patch.dump());
  EXPECT_EQ(stale.status_code, 409) << stale.body;
  const auto stale_json = nlohmann::json::parse(stale.body);
  EXPECT_EQ(stale_json.at("code").get<std::string>(), "stale_revision_conflict");
  EXPECT_EQ(stale_json.at("revision").get<std::uint64_t>(), 2u);

  const auto generated_patch = nlohmann::json{{"schema", "graphx.dashboard.config_update.v1"},
                                              {"command_id", "cmd-step2-patch-3"},
                                              {"expected_revision", 2},
                                              {"pointer", "/fhss/scenario/active_frequency_indices"},
                                              {"value", nlohmann::json::array({24, 28, 32, 36})},
                                              {"apply", "staged"}};
  const auto generated = HttpRequest(server_->BoundPort(), "PATCH", "/api/v1/config", generated_patch.dump());
  EXPECT_EQ(generated.status_code, 409) << generated.body;
  const auto generated_json = nlohmann::json::parse(generated.body);
  EXPECT_EQ(generated_json.at("code").get<std::string>(), "derived_field_read_only");
  EXPECT_EQ(generated_json.at("pointer").get<std::string>(), "/fhss/scenario/active_frequency_indices");
  EXPECT_EQ(generated_json.at("details").at("authoritative_pointer").get<std::string>(),
            "/fhss/scenario");
}

TEST_F(DashboardServerStep2Test, ValidationErrorsHaveStableLevelsAndShape) {
  const auto invalid_patch = nlohmann::json{{"schema", "graphx.dashboard.config_update.v1"},
  ...
  const auto response = HttpRequest(server_->BoundPort(), "PATCH", "/api/v1/config", invalid_patch.dump());
  EXPECT_EQ(response.status_code, 200) << response.body;
  const auto json = nlohmann::json::parse(response.body);
  EXPECT_EQ(json.at("schema").get<std::string>(), "graphx.dashboard.config_validation.v1");
  ...
  EXPECT_EQ(error.at("pointer").get<std::string>(), "/fhss/scenario/messages/0/pulses/0/frequency_index");
  EXPECT_FALSE(error.at("code").get<std::string>().empty());
  EXPECT_FALSE(error.at("message").get<std::string>().empty());
}

TEST_F(DashboardServerStep2Test, PatchExportAndOperationLifecycleAreContracted) {
  const auto patch = nlohmann::json{{"schema", "graphx.dashboard.config_update.v1"},
                                    {"command_id", "cmd-step2-export-patch"},
                                    {"expected_revision", 1},
                                    {"pointer", "/fhss/scenario/iq_center_frequency_hz"},
                                    {"value", 1240000001.0},
                                    {"apply", "staged"}};
  const auto patch_response = HttpRequest(server_->BoundPort(), "PATCH", "/api/v1/config", patch.dump());
  ...
  const auto export_request = nlohmann::json{{"schema", "graphx.dashboard.config_export.v1"},
                                             {"command_id", "cmd-step2-export-1"},
                                             {"expected_revision", 2},
                                             {"output_path", export_path.string()},
                                             {"resource", "effective"}};
  const auto export_response = HttpRequest(server_->BoundPort(), "POST", "/api/v1/config/export",
                                           export_request.dump());
  ...
  const auto operation = HttpRequest(server_->BoundPort(), "GET", "/api/v1/operations/" + operation_id);
  ...
  EXPECT_EQ(operation_json.at("result").at("output_path").get<std::string>(), export_path.string());

  const auto cancel = HttpRequest(server_->BoundPort(), "POST",
  ...
  EXPECT_EQ(cancel_json.at("code").get<std::string>(), "operation_not_terminal");

  const auto deleted = HttpRequest(server_->BoundPort(), "DELETE", "/api/v1/operations/" + operation_id);
  EXPECT_EQ(deleted.status_code, 204) << deleted.body;

  const auto after_delete = HttpRequest(server_->BoundPort(), "GET", "/api/v1/operations/" + operation_id);
  ...
  EXPECT_EQ(after_delete_json.at("code").get<std::string>(), "operation_not_found_or_expired");
}

TEST(GraphConfigurationServiceStep2Test, ExportReplayAndFailureInjectionBehaveDeterministically) {
  auto config = LoadJsonFile(std::filesystem::path(DSP_FHSS_CHANNELIZED_CONFIG_PATH));
  graph::dashboard::GraphConfigurationService service(config);
  service.SetArtifactRoot(MakeTempArtifactDirectory("graphx_dashboard_step2_service_artifacts"));
  ...
  const auto export_path = std::filesystem::temp_directory_path() / "graphx_dashboard_step2_service_artifacts" /
                           "replay" / "effective.json";
  const auto request = nlohmann::json{{"schema", "graphx.dashboard.config_export.v1"},
  ...
  EXPECT_EQ(replay.at("operation_id").get<std::string>(), first.at("operation_id").get<std::string>());

  auto mismatched_request = request;
  mismatched_request["output_path"] = (export_path.parent_path() / "other.json").string();
  const auto mismatched = service.ExportConfig(mismatched_request);
  EXPECT_EQ(mismatched.at("schema").get<std::string>(), "graphx.dashboard.error.v1");
  EXPECT_EQ(mismatched.at("code").get<std::string>(), "idempotency_key_reused_with_different_payload");

  service.SetValidationInjectorForTesting([](const nlohmann::json &request_body) -> std::optional<graph::dashboard::GraphConfigurationService::ValidationError> {
    if (request_body.value("command_id", std::string{}) == "cmd-enospc") {
      return graph::dashboard::GraphConfigurationService::ValidationError{.level = "construction",
                                                                          .node_id = "graph",
  ...
  const auto failure_request = nlohmann::json{{"schema", "graphx.dashboard.config_export.v1"},
  ...
  EXPECT_EQ(failure.at("result").at("code").get<std::string>(), "enospc_during_export");
  ...
  EXPECT_EQ(expired.at("schema").get<std::string>(), "graphx.dashboard.error.v1");
  EXPECT_EQ(expired.at("code").get<std::string>(), "operation_not_found_or_expired");
}
