...
VERSION = "graphx.fhss.phase3-independent.v3"
...
def validate_profile(profile: dict[str, Any]) -> None:
    ...
    if profile.get("schema") != "graphx.fhss.phase3-validation-profile.v5" or profile.get("version") != 5:
        raise ValueError("only the post-v4-failure v5 profile is accepted")
    ...
    _exact_keys(frozen, {"matching_tolerance_samples", "frequency_match",
                         ...
                         "matrix_sha256", "seed_partitions_sha256", "gates"}, "frozen")
    ...
    if freeze.get("status") != "frozen_before_held_out_evaluation":
        raise ValueError("v5 profile is not frozen for held-out evaluation")
    if (not isinstance(freeze.get("sha256"), str) or
            len(freeze["sha256"]) != 64):
        raise ValueError("v5 frozen-section hash is malformed")
...
def run_receiver(executable: Path, base_config: Path, plugins: Path,
    ...
    command = [str(executable), "--graph-config", str(patched), "--plugin-dir", str(plugins),
               "--summary-json", str(summary), "--effective-config-json", str(effective), "--executor-timeout-s", "30"]
    timed_out = False
    try:
...
def evaluate(profile: dict[str, Any], executable: Path, base_config: Path,
    ...
    def raw_document() -> dict[str, Any]:
        return {"schema": "graphx.fhss.phase3-raw-evaluation.v3",
...
def aggregate(profile: dict[str, Any], raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema": "graphx.fhss.phase3-report.v3", "claim_level": profile["claim_level"],
