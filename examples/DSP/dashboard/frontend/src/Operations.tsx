...
export function Operations({ refreshToken }: { refreshToken: number }) {
  ...
  const configuration = data.configuration as ConfigurationResource | undefined;
  const revision = configuration?.config_revision ?? 0;
  const configurationEtag = configuration?.httpEtag ?? '';
  const jobs = data.jobs as JobHistoryResource | undefined;
  const investigations = data.investigations as InvestigationHistoryResource | undefined;
