...
export function Visualization({ value, selectedChannel, onChannel }: {
  value: unknown;
  ...
  return <article className="visualization"><h3>FHSS Schedule</h3><p>{String(document.fixture_label ?? 'Synthetic schedule unavailable.')}</p>
    ...
    <h3>64-Channel Heatmap</h3><p>This heatmap is evaluator-side expected truth. Selecting a channel changes presentation focus only.</p>
    <div className="heatmap" aria-label="Expected pulse counts by physical channel">
      {channels.map((channel) => {
        const count = Number(channel.expected_pulse_count ?? 0);
        const channelIndex = Number(channel.channel_index);
        return <button type="button" key={String(channel.channel_index)}
          aria-pressed={selectedChannel === channelIndex}
          onClick={() => onChannel?.(channelIndex)}
          style={{ '--load': count / maximum } as CSSProperties}>
          <strong>Ch {String(channel.channel_index)}</strong><span>{count} expected pulses</span>
        </button>;
      })}
...
export function Operations({ refreshToken, selectedPhysicalChannel, onPhysicalChannel, onObservation }: {
  ...
  }, [selectedPhysicalChannel, spectrumChannel]);
  return <section className="operations" aria-labelledby="operations-heading">
    ...
    <p role="status" aria-live="polite">{status}</p><div className="operation-grid">
      ...
      <ReceiverEvidence expected={expectedTruth} observation={observations} comparison={comparison} spectrum={spectrum} channel={spectrumChannel}
        onChannel={(channel) => { setSpectrumChannel(channel); onPhysicalChannel?.(channel); }} />
      <MetricsActivity value={metrics} /><Visualization value={data.visualization}
        selectedChannel={spectrumChannel}
        onChannel={(channel) => { setSpectrumChannel(channel); onPhysicalChannel?.(channel); }} />
