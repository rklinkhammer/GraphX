...
TEST_F(GraphxGotchaToCrsdCliTest, UnsupportedMatAndCrsdModeFailClearly) {
    ...
    const auto crsd_mode = RunCommand(
        CliBase() +
        " --input-dir " + ShellQuote(input_dir) +
        " --output-dir " + ShellQuote(Path("out-e")) +
        " --collection-id c5 --max-output-size-mb 1 --sort lexical --mode crsd 2>&1");
    EXPECT_NE(crsd_mode.exit_code, 0);
    EXPECT_NE(crsd_mode.output.find("crsd_mode_not_implemented"), std::string::npos);
}
