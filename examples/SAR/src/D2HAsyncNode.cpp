...
void D2HAsyncNode::Configure(const graph::JsonView& cfg) {
    auto config = config_;
...
graph::JsonView D2HAsyncNode::GetParameters() const {
...
graph::JsonView D2HAsyncNode::GetParameterDescription(
...
std::vector<std::string> D2HAsyncNode::GetParameterNames() const {
    return {
        "override_backend",
        "backend_id",
        "backend",
...
void D2HAsyncNode::SetConfig(const D2HAsyncConfig& config) {
    config_ = config;
}

const D2HAsyncConfig& D2HAsyncNode::GetConfig() const noexcept {
    return config_;
}
